...
func main() {
	...
	// 핸들러 초기화
	writeHandler := handler.NewWriteHandler(db, collector)
	...
	// 로그 INSERT API
	router.HandleFunc("/logs", writeHandler.InsertLog).Methods("POST")
	router.HandleFunc("/logs/batch", writeHandler.InsertBatchLogs).Methods("POST")
	...
	go func() {
		log.Printf("Write server listening on port %s", serverPort)
